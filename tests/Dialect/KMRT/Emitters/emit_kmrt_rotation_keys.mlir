...
// CHECK: CiphertextT test_static_rotation_keys(
// CHECK-SAME:    CryptoContextT [[CC:[^,]*]],
// CHECK-SAME:    CiphertextT [[CT:[^)]*]]
// CHECK-SAME:  ) {
// CHECK-NEXT:      {{.*}}int64_t [[idx5:.*]] = 5;
// CHECK-NEXT:      RotKey [[rk1:.*]] = keymem_rt.deserializeKey(5{{.*}});
...
// CHECK-NEXT:      {{.*}}int64_t [[idx16:.*]] = 16;
// CHECK-NEXT:      RotKey [[rk2:.*]] = keymem_rt.deserializeKey(16{{.*}});
// CHECK-NEXT:      auto [[ct2:.*]] = [[CC]]->EvalRotate([[ct1]], [[idx16]]);
// CHECK-NEXT:      LOG_ROT([[ct1]],{{.*}});
// CHECK-NEXT:      LOG_CT([[ct2]],{{.*}});
// CHECK-NEXT:      keymem_rt.clearKey([[rk2]]);
// CHECK-NEXT:      return [[ct2]];
// CHECK-NEXT:  }
module attributes {backend.openfhe, scheme.ckks} {
  func.func @test_static_rotation_keys(%cc: !cc, %ct: !ct) -> !ct {
...
// CHECK: CiphertextT test_dynamic_rotation_keys(
// CHECK-SAME:    CryptoContextT [[CC:[^,]*]],
// CHECK-SAME:    CiphertextT [[CT:[^,]*]],
...
// CHECK-NEXT:      return [[ct1]];
// CHECK-NEXT:  }
module attributes {backend.openfhe, scheme.ckks} {
  func.func @test_dynamic_rotation_keys(%cc: !cc, %ct: !ct, %idx: index) -> !ct {
...
// CHECK: CiphertextT test_mixed_rotation_keys(
// CHECK-SAME:    CryptoContextT [[CC:[^,]*]],
// CHECK-SAME:    CiphertextT [[CT:[^,]*]],
// CHECK-SAME:    size_t [[IDX:[^)]*]]
// CHECK-SAME:  ) {
// CHECK-NEXT:      {{.*}}int64_t {{.*}} = 5;
// CHECK-NEXT:      RotKey {{.*}} = keymem_rt.deserializeKey(5{{.*}});
// CHECK-NEXT:      auto [[ct1:.*]] = [[CC]]->EvalRotate([[CT]], {{.*}});
// CHECK-NEXT:      LOG_ROT([[CT]],"%ct");
// CHECK-NEXT:      LOG_CT([[ct1]],"%ct_0");
// CHECK-NEXT:      keymem_rt.clearKey({{.*}});
// CHECK-NEXT:      RotKey {{.*}} = keymem_rt.deserializeKey([[IDX]]);
...
// CHECK-NEXT:  }
module attributes {backend.openfhe, scheme.ckks} {
  func.func @test_mixed_rotation_keys(%cc: !cc, %ct: !ct, %idx: index) -> !ct {
    %c5 = arith.constant 5 : i64
