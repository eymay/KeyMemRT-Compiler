...
FailureOr<RingAttr> getEncRingFromFuncOp(func::FuncOp op) {
  ...
  for (auto argTy : argTypes) {
    ...
    if (auto argCtTy = dyn_cast<lwe::LWECiphertextType>(argTy)) {
...
FailureOr<RingAttr> getDecRingFromFuncOp(func::FuncOp op) {
  ...
  for (auto resultTy : resultTypes) {
    ...
    // Check that parameters are unique
    if (auto resultCtTy = dyn_cast<lwe::LWECiphertextType>(resultTy)) {
...
LogicalResult generateEncryptionFunc(func::FuncOp op,
  ...
  Type encryptionKeyType =
      usePublicKey ? (Type)lwe::LWEPublicKeyType::get(
  ...
  for (size_t i = 0; i < encFuncArgTypes.size(); ++i) {
    ...
    if (auto resultCtTy = dyn_cast<lwe::LWECiphertextType>(resultTy)) {
      auto plaintextTy = lwe::LWEPlaintextType::get(
          op.getContext(), resultCtTy.getPlaintextSpace());
...
LogicalResult generateDecryptionFunc(func::FuncOp op,
                                     ...
                                     RingAttr ring,
                                     ImplicitLocOpBuilder &builder) {
  Type decryptionKeyType = lwe::LWESecretKeyType::get(
  ...
  for (size_t i = 0; i < decFuncResultTypes.size(); ++i) {
    ...
    if (auto argCtTy = dyn_cast<lwe::LWECiphertextType>(argTy)) {
      auto plaintextTy = lwe::LWEPlaintextType::get(
          op.getContext(), argCtTy.getPlaintextSpace());
...
LogicalResult convertFunc(func::FuncOp op, bool usePublicKey) {
  ...
  for (auto val : op.getArguments()) {
    ...
    if (auto argCtTy = dyn_cast<lwe::LWECiphertextType>(argTy)) {
      ...
      // application_data was removed from LWECiphertextType; use the
      // plaintext ring's coefficient type as a stand-in for the original
      // message element type.
      Type messageTy =
          argCtTy.getPlaintextSpace().getRing().getCoefficientType();
      if (failed(generateEncryptionFunc(op, encFuncName, {messageTy}, {argCtTy},
                                        usePublicKey, ringEnc, builder))) {
        return failure();
      }
  ...
  for (size_t i = 0; i < returnTypes.size(); ++i) {
    ...
    if (auto returnCtTy = dyn_cast<lwe::LWECiphertextType>(returnTy)) {
      ...
      // message element type.
      Type messageTy =
          returnCtTy.getPlaintextSpace().getRing().getCoefficientType();
      if (failed(generateDecryptionFunc(op, decFuncName, {returnCtTy},
                                        {messageTy}, ringDec, builder))) {
        return failure();
      }
      // insertion point is inside func, move back out
      builder.setInsertionPointToEnd(module.getBody());
